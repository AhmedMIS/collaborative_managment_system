...
<table style="margin:0 auto">
   <tr>
    <td>Meeting Date</td>
    <td style="border-right:none"><input type="text" name="meetingdate" id="meetingdate" /><?php if(isset($errors['meeting_date'])) echo '<li>'.$errors['meeting_date'] . '</li>'?></td>
  </tr>
  <tr>
    <td>Send To</td>
    <td style="border-right:none"><select name="userid" class="chosen-select-full-width"  >
							<?php
								include_once("../php/classes/user.php");
								$user_obj = new user();
								$users = $user_obj->getUsersByType1(3,1,$_SESSION['user']['id']);
								if($users){
