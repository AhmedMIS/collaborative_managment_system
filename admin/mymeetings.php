		 ...
		 <div id="body">
			...
			?>
			 ...
			 while( $meeting = mysql_fetch_assoc($meetings) ){ $meetings_arr[] = Array('id'=>$meeting['id'],'description'=>$meeting['description'],'title'=>$meeting['title'],'meetingdate'=>$meeting['meetingdate'],'created'=>$meeting['created'],'status'=>$meeting['status']); }?>
				...
				<form action="" method="post">
					<div id="row_filter" style="display:<?php echo (($_SERVER['REQUEST_METHOD'] == 'POST'))?"block":"none"; ?>">
						<div class="project_row info">
							...
							<div class="project_cell _text"><select class="chosen-select" name="status"><option ></option><option value="1">Held</option></select></div>
				...
				<div class="project_row <?php $classes = Array('block','held','active'); echo $classes[$meeting['status']];?>" title="<?php echo ($meeting['status'] == 1)?"This meeting has been stopped.":$meeting['description']; ?>">
                    ...
                    <a href="viewmeetings.php?meetingid=<?php echo $meeting['id']; ?>">
						<span title="View Meeting" class="ui-icon ui-icon-clipboard"></span></a>
