		 ...
		 <div id="body">
			...
			?>
			 ...
			 while( $project = mysql_fetch_assoc($projects) ){ $projects_arr[] = Array('id'=>$project['id'],'description'=>$project['description'],'title'=>$project['title'],'fieldtitle'=>$project['fieldtitle'],'fname'=>$project['fname'],'lname'=>$project['lname'],'startingdate'=>$project['startingdate'],'endingdate'=>$project['endingdate'],'status'=>$project['status']); }?>
				...
				<form action="" method="post">
					<div id="row_filter" style="display:<?php echo (($_SERVER['REQUEST_METHOD'] == 'POST'))?"block":"none"; ?>">
						<div class="project_row info">
							...
							<div class="project_cell _text"><?php
									include_once("../php/classes/user.php");
									$user_obj = new user();
									$users = $user_obj->getUsersByType(2,1);/* 1 = user status, 1 = roleid, 1 = Project manager  */	
									if($users){ ?>
										<select class="chosen-select" name="userid"><option></option>
										<?php
											while( $user = mysql_fetch_assoc($users) ){
												echo '<option value="'.$user['id'].'" '.((isset($_POST) && !empty($_POST['userid']) && $_POST['userid'] == $user['id'] )?"selected":"").'>'.$user['fname'].' '.$user['lname'].'</option>';
										} 
									?> </select> 
								<?php }	?>
							</div>
				...
				<div class="project_row <?php echo ($project['status'] == 0)?"block":"active"; ?>" title="<?php echo $project['description']; ?>">
					...
					<div class="project_cell _link">
						...
						<a href="update_project.php?projectid=<?php echo $project['id']; ?>"><span title="Update Project" class="ui-icon ui-icon-refresh"></span></a>
